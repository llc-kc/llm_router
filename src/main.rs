...
use modes::{Worker, cache_router, mirror, split};
...
struct Args {
    ...
    /// 例如: [{"url":"http://127.0.0.1:8000"},{"url":"http://127.0.0.1:8001","cache_hit_rate":0.8}]
    #[arg(long, default_value = "")]
    workers: String,

    /// 运行模式：mirror（镜像模式）、split（分流模式）、cache_route（基于KV cache命中率的路由模式）
    ...
    /// Tokenizer模型文件路径，用于cache_route模式
    #[arg(long, default_value = "")]
